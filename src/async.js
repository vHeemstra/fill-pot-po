...
function fillPotPo(cb, options) {
	if (typeof cb !== 'function') {
		throw new PluginError('fillPotPo() requires a callback function as first parameter');
	}
	...
	Promise.all( options.potSources.map(pot_file => {
		const pot_filepath = Vinyl.isVinyl(pot_file) ? pot_file.relative : pot_file;
		...
		return new Promise((resolve, reject) => {
			...
			const po_results = await Promise.all( po_files.map(po_file => {
				return new Promise((resolve, reject) => {
